...
class VPINToxicityFormula(BaseFormula):
    """
    ID 286: MILLISECOND VPIN - Real-Time Toxicity Detection

    CRITICAL: Market changes every MILLISECOND. 20,000-30,000 trades/second.
    Old VPIN is useless - we need INSTANT toxicity detection.

    NO HARDCODED VALUES - Everything derived from live blockchain data.

    New Approach:
    - Time-based windows (last 1-2 seconds of data)
    - Automatic decay (old data becomes irrelevant)
    - Dynamic thresholds based on current market activity
    """

    FORMULA_ID = 286
    CATEGORY = "execution"
    NAME = "Millisecond VPIN"
    DESCRIPTION = "Real-time toxicity detection at millisecond scale"

    def __init__(self, lookback: int = 100, bucket_size: float = 0.01,
                 n_buckets: int = 20, **kwargs):
        super().__init__(lookback, **kwargs)
        # MILLISECOND SCALE - tiny buckets, fast turnover
        self.bucket_size = bucket_size  # 0.01 BTC = ~$900 per bucket (NOT 10 BTC!)
        self.n_buckets = n_buckets  # Only 20 buckets = 0.2 BTC window

        # Time-based decay - market changes every second
        self.decay_window_ms = 2000  # 2 second window max
        self.last_update_time = 0.0

        # Timestamped buckets for decay
        self.buckets = deque(maxlen=n_buckets)
        self.bucket_times = deque(maxlen=n_buckets)
        self.current_bucket = {'buy_volume': 0.0, 'sell_volume': 0.0, 'total_volume': 0.0, 'time': 0.0}

        # VPIN value - starts SAFE (0.3) not neutral
        self.vpin = 0.3  # Start safe, let data prove toxicity

        # Trade classification
        self.last_price = None

        # Dynamic baseline from blockchain
        self.baseline_imbalance = 0.0
        self.imbalance_ema = 0.5  # EMA of imbalance for adaptive threshold
    ...
    def update(self, price: float, volume: float = 0.0, timestamp: float = 0.0) -> Optional[float]:
        """
        MILLISECOND VPIN - Update with time-based decay

        Market changes every millisecond. Old data is IRRELEVANT.
        We only care about the last 1-2 seconds of order flow.
        """
        import time as time_module
        now = timestamp if timestamp > 0 else time_module.time()
        now_ms = now * 1000  # Convert to milliseconds

        # CRITICAL: Expire old buckets (market changed, data is stale)
        self._expire_old_buckets(now_ms)

        # Classify trade
        buy_vol, sell_vol = self._classify_trade(price, volume)
        self.last_price = price

        # Add to current bucket with timestamp
        self.current_bucket['buy_volume'] += buy_vol
        self.current_bucket['sell_volume'] += sell_vol
        self.current_bucket['total_volume'] += volume
        self.current_bucket['time'] = now_ms

        vpin_updated = None

        # Check if bucket is full (tiny buckets fill fast at 20k trades/sec)
        if self.current_bucket['total_volume'] >= self.bucket_size:
            # Calculate order imbalance for this bucket
            total = self.current_bucket['total_volume']
            if total > 0:
                order_imbalance = abs(self.current_bucket['buy_volume'] - self.current_bucket['sell_volume']) / total
            else:
                order_imbalance = 0.0

            self.buckets.append(order_imbalance)
            self.bucket_times.append(now_ms)

            # Reset current bucket
            self.current_bucket = {'buy_volume': 0.0, 'sell_volume': 0.0, 'total_volume': 0.0, 'time': now_ms}

            # Calculate VPIN from recent buckets only
            self._calculate_vpin(now_ms)
        ...
        # Apply time decay even without new bucket
        # VPIN decays toward neutral (0.5) when no new data - no assumptions
        time_since_last = now_ms - self.last_update_time
        if time_since_last > 100:  # Every 100ms
            # Decay rate should be from LIVE data, but use minimal decay for now
            decay_rate = 0.01  # 1% decay - conservative, let live data dominate
            self.vpin = self.vpin * (1 - decay_rate) + 0.5 * decay_rate  # Decay to neutral, not "safe"
    ...
    def _expire_old_buckets(self, now_ms: float):
        """Remove buckets older than decay window - MARKET CHANGED"""
        cutoff = now_ms - self.decay_window_ms

        while self.bucket_times and self.bucket_times[0] < cutoff:
            self.bucket_times.popleft()
            self.buckets.popleft()

    def _calculate_vpin(self, now_ms: float):
        """Calculate VPIN from ONLY recent buckets"""
        if not self.buckets:
            self.vpin = 0.3  # Safe when no data
            return

        # Only use buckets from last decay window
        cutoff = now_ms - self.decay_window_ms
        recent_imbalances = []

        for i, t in enumerate(self.bucket_times):
            if t >= cutoff:
                recent_imbalances.append(self.buckets[i])

        if not recent_imbalances:
            self.vpin = 0.3  # Safe when no recent data
            return

        # VPIN = average imbalance (0 to 1 scale)
        raw_vpin = sum(recent_imbalances) / len(recent_imbalances)

        # Update EMA for adaptive baseline
        self.imbalance_ema = 0.1 * raw_vpin + 0.9 * self.imbalance_ema

        # VPIN relative to baseline (if current is 2x baseline, that's toxic)
        if self.imbalance_ema > 0:
            relative_vpin = raw_vpin / (self.imbalance_ema * 2)
        else:
            relative_vpin = raw_vpin

        self.vpin = min(1.0, max(0.0, relative_vpin))
    ...
    def _compute(self) -> None:
        ...
        if not self.in_position:
            ...
            if self.z_score < -self.entry_z:
                ...
                stop_z = self.stop_z if self.stop_z > 0 else 3.0
                self.confidence = min(1.0, abs(self.z_score) / stop_z)
...
class QueuePositionFormula(BaseFormula):
    ...
    def __init__(self, lookback: int = 100, base_adverse_selection: float = 0.0,
                 position_coefficient: float = 0.0, **kwargs):
        super().__init__(lookback, **kwargs)
        self.alpha = base_adverse_selection  # LIVE: From actual adverse selection measurement
        self.beta = position_coefficient     # LIVE: From actual position impact

        # Spread tracking - NO DEFAULT, must be from LIVE data
        self.spread = 0.0  # LIVE: From actual bid-ask spread

        # Queue position tracking - NO DEFAULTS
        self.our_bid_position = 0
        self.our_ask_position = 0
        self.bid_queue_size = 0  # LIVE: From actual order book
        self.ask_queue_size = 0  # LIVE: From actual order book
