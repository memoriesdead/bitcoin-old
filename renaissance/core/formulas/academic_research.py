...
class MasterAcademicScaler(BaseFormula):
    ...
    def _compute(self) -> None:
        ...
        # 6. Calculate expected returns
        base_edge = 0.00019  # 0.019% per trade
        daily_trades = self.optimal_trades_per_day

        # Edge amplification from Grinold-Kahn
        amplification = np.sqrt(daily_trades * 365) if daily_trades > 0 else 1
        amplified_edge = base_edge * min(amplification / 1000, 10)  # Cap amplification
