...
USD TRANSLATIONS: All BTC values are translated to USD for clarity
"""
...
class BlockchainMarketState:
    ...
    # USD TRANSLATION - Derived BTC price for understanding values
    btc_price_usd: float = 0.0      # Current BTC price in USD (derived from blockchain)
...
class BlockchainMarketData:
    ...
    NOW WITH USD TRANSLATIONS for better understanding!
    ...
    def __init__(
        ...
        # USD PRICE TRACKING - Will be updated by price engine callback
        self._btc_price_usd = 0.0

    def set_btc_price(self, price_usd: float):
        """Set the current BTC price in USD for translations"""
        if price_usd > 0:
            self._btc_price_usd = price_usd
            self.state.btc_price_usd = price_usd
    ...
    async def start(self):
        ...
        print("USD TRANSLATIONS enabled for better value understanding!")
        print()
    ...
    def _format_usd(self, btc_value: float) -> str:
        """Format BTC value as USD string"""
        if self._btc_price_usd <= 0:
            return "---"
        usd = btc_value * self._btc_price_usd
    ...
    async def _signal_generator(self):
        ...
        print()
        print("BLOCKCHAIN MARKET SIGNALS ACTIVE - WITH USD TRANSLATIONS")
        print("-" * 100)
        # Header with both raw and USD columns
        print(f"{'Time':>6} | {'TX/s':>5} | {'Vol 5m BTC':>10} | {'Vol USD':>10} | "
              f"{'Fee':>4} | {'Mempool':>8} | {'Whales':>6} | "
              f"{'Signal':>10} | {'Str':>5} | {'BTC Price':>12}")
        print("-" * 100)
        ...
        while self.running:
            ...
            if now - self._last_signal_time >= self.signal_interval:
                ...
                # Calculate USD translations
                vol_5m_usd = self._format_usd(self.state.btc_volume_5m)
                btc_price_str = f"${self._btc_price_usd:,.0f}" if self._btc_price_usd > 0 else "calibrating"

                # Print status with BOTH raw blockchain AND USD translations
                elapsed = int(now - self._start_time)
                print(f"T+{elapsed:4d}s | {self.state.tx_rate:>5.1f} | "
                      f"{self.state.btc_volume_5m:>10.1f} | {vol_5m_usd:>10} | "
                      f"{self.state.fee_fast:>4} | {self.state.mempool_size:>8,} | "
                      f"{self.state.whale_tx_count:>6.0f} | "
    ...
    def _generate_signal(self):
        """
        Generate BIDIRECTIONAL trading signal from blockchain state

        BULLISH conditions (BUY/STRONG_BUY):
        - Rising fees = increasing demand
        - Rising volume = accumulation
        - Low mempool = calm market, good for longs

        BEARISH conditions (SELL/STRONG_SELL):
        - Fee SPIKE (>3x baseline) = panic selling
        - Falling fees = decreasing demand
        - High mempool congestion = panic
        - Whale activity + falling price = distribution
        """
        pressure = 0.0
        weights_sum = 0.0

        # ================================================================
        # 1. FEE ANALYSIS - BIDIRECTIONAL
        # ================================================================
        # High fees can be BULLISH (demand) OR BEARISH (panic selling)
        # Key: Look at fee TREND and SPIKE vs baseline
        if self.state.fee_fast > 0:
            # Check for fee SPIKE (panic indicator)
            baseline_fee = getattr(self, '_baseline_fee', self.state.fee_fast)
            if not hasattr(self, '_baseline_fee'):
                self._baseline_fee = self.state.fee_fast

            fee_ratio = self.state.fee_fast / max(1, baseline_fee)

            if fee_ratio > 3.0:
                # EXTREME fee spike = PANIC SELLING = BEARISH
                fee_signal = -1.0
            elif fee_ratio > 2.0:
                # Moderate fee spike = caution
                fee_signal = -0.5
            elif self.state.fee_fast >= self.HIGH_FEE_THRESHOLD:
                # High but stable fees = demand = bullish
                fee_signal = 0.5
            elif self.state.fee_fast <= self.LOW_FEE_THRESHOLD:
                # Very low fees = low activity = slight bearish
                fee_signal = -0.3
            else:
                # Normal fees
                fee_signal = 0.0

            pressure += fee_signal * 0.25
            weights_sum += 0.25

        # ================================================================
        # 2. FEE TREND - BIDIRECTIONAL
        # ================================================================
        # Rising fees = bullish, Falling fees = bearish
        fee_trend_signal = np.clip(self.state.fee_trend * 5, -1, 1)
        pressure += fee_trend_signal * 0.15
        weights_sum += 0.15

        # ================================================================
        # 3. VOLUME TREND - BIDIRECTIONAL
        # ================================================================
        # Rising volume = bullish, Falling volume = bearish
        vol_trend_signal = np.clip(self.state.volume_trend * 3, -1, 1)
        pressure += vol_trend_signal * 0.15
        weights_sum += 0.15

        # ================================================================
        # 4. MEMPOOL ANALYSIS - BIDIRECTIONAL
        # ================================================================
        # HIGH mempool = congestion = PANIC = BEARISH
        # LOW mempool = calm = good for longs = BULLISH
        baseline_mempool = getattr(self, '_baseline_mempool', self.state.mempool_size)
        if not hasattr(self, '_baseline_mempool') and self.state.mempool_size > 0:
            self._baseline_mempool = self.state.mempool_size

        if baseline_mempool > 0 and self.state.mempool_size > 0:
            mempool_ratio = self.state.mempool_size / baseline_mempool

            if mempool_ratio > 1.5:
                # HIGH mempool = congestion = PANIC = BEARISH
                mempool_signal = -0.8 * min(1.0, (mempool_ratio - 1.0))
            elif mempool_ratio < 0.7:
                # LOW mempool = calm = BULLISH
                mempool_signal = 0.5 * (1.0 - mempool_ratio)
            else:
                mempool_signal = 0.0

            pressure += mempool_signal * 0.2
            weights_sum += 0.2

        # Also consider growth rate
        if self.state.mempool_growth_rate != 0:
            # Rapid mempool GROWTH = panic = BEARISH
            # Rapid mempool SHRINK = clearing = BULLISH
            growth_signal = -np.clip(self.state.mempool_growth_rate / 50, -1, 1)
            pressure += growth_signal * 0.1
            weights_sum += 0.1

        # ================================================================
        # 5. WHALE ACTIVITY - BIDIRECTIONAL
        # ================================================================
        # High whale activity amplifies the signal direction
        # If other signals are bearish, whales amplify bearishness
        whale_multiplier = 1.0 + self.state.whale_activity * 0.3

        # ================================================================
        # 6. BLOCK INTERVAL - REDUCED WEIGHT (less predictive)
        # ================================================================
        if self.state.block_interval > 0:
            # Normal is ~600s. Block timing has limited predictive value
            # Don't heavily weight this
            if self.state.block_interval > 1200:
                block_signal = 0.2  # Slight congestion signal
            elif self.state.block_interval < 300:
                block_signal = -0.2  # Fast blocks = clearing
            else:
                block_signal = 0
            pressure += block_signal * 0.1
            weights_sum += 0.1
    ...
    def get_stats(self) -> dict:
        ...
        return {
            'state': {
                ...
                'btc_volume_5m_usd': self.state.btc_volume_5m * self._btc_price_usd if self._btc_price_usd > 0 else 0,
                ...
                'btc_price_usd': self._btc_price_usd,
            },
...
async def test_blockchain_market(duration: int = 60):
    ...
    if market._btc_price_usd > 0:
        print(f"  USD Volume 5m: ${state.btc_volume_5m * market._btc_price_usd:,.0f}")
    ...
    if whales:
        ...
        for tx in whales:
            usd_val = tx.value_btc * market._btc_price_usd if market._btc_price_usd > 0 else 0
            print(f"  {tx.value_btc:>10.2f} BTC (${usd_val:>12,.0f}) | {tx.fee_rate:.1f} sat/vB | {tx.txid[:16]}...")
